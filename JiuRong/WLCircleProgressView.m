...
- (instancetype)initWithFrame:(CGRect)frame circlesSize:(CGRect)size
{
    ...
    if (self) {
        self.circlesSize = CGRectMake(30, 5, 30, 5);
//        [self initView];
    }
    return self;
}
...
{
    self.circlesSize = CGRectMake(30, 4, 30, 4);
...
-(void)addBackCircleWithSize:(CGFloat)radius lineWidth:(CGFloat)lineWidth{
    self.backCircle = [self createShapeLayerWithSize:radius lineWith:lineWidth color:[UIColor greenColor]];
...
-(void)addForeCircleWidthSize:(CGFloat)radius lineWidth:(CGFloat)lineWidth{
    ...
    self.foreCircle.strokeColor = [UIColor redColor].CGColor;
